...
class SmartRouter:
    ...
    COMPLEX_KEYWORDS = [
        ...
        '数据库', 'SQL', 'API', '设计', '优化', '性能', '安全', '部署',
        '测试', '单元测试', '集成', 'CI', 'CD', '并发', '线程', '异步',
        '网络', '协议', '加密', '认证', '授权', '缓存', '索引', '事务',
        'middleware', 'pipeline', 'workflow', 'orchestration', '微服务',
        'docker', 'kubernetes', 'k8s', '容器', '编排',
        'framework', 'architecture', 'refactor', '重构',
        'code', 'programming', 'function', 'class', 'module',
        'implement', 'implementation', 'design pattern',
    ]
    SIMPLE_KEYWORDS = [
        '你好', 'hi', 'hello', '天气', '今天', '早上', '晚上', '下午',
        '谢谢', '感谢', '再见', '拜拜', '嗯', '好', 'ok',
        '名字', '你是谁', '你能做什么',
    ]
    ...
    def classify(self, query: str) -> str:
        ...
        # 含代码块标记
        if '```' in q or 'def ' in q or 'class ' in q or 'import ' in q:
            return 'complex'
        
        # 复杂关键词匹配
        q_lower = q.lower()
        complex_score = sum(1 for kw in self.COMPLEX_KEYWORDS if kw.lower() in q_lower)
        simple_score = sum(1 for kw in self.SIMPLE_KEYWORDS if kw.lower() in q_lower)
