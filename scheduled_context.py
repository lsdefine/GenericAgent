...
def record_scheduled_output(task, result, root=ROOT, keep=20):
    task_name = _field(task, "定时任务")
    if not task_name:
        return
    report_path = _field(task, "报告路径")
    body = _read_report(report_path) or result
    entry = {
        "ts": time.time(),
        "task": task_name,
        "report": report_path,
    ...
    if os.path.exists(path):
        with open(path, encoding="utf-8", errors="replace") as f:
            rows = [line for line in f.readlines() if line.strip()][-keep + 1:]
...
def recent_scheduled_context(root=ROOT, max_age=6 * 3600, limit=3):
    path = os.path.join(root, "temp", "scheduled_context.jsonl")
