...
- [kevinchennewbee/PenglaiAgent](https://github.com/kevinchennewbee/PenglaiAgent) — Penglai (蓬莱): a personal-butler distribution of GA with zero kernel changes — ten-minute wizard (scan-to-create Feishu bot), one-line curl/Docker/pip install, voice emotion, deterministic red-line safety, opt-in proactive companion.
...
- [kevinchennewbee/PenglaiAgent](https://github.com/kevinchennewbee/PenglaiAgent) —— 蓬莱：GA 的个人管家发行版（内核零改动）——十分钟向导（飞书扫码建应用）、curl/Docker/pip 一行部署、语音情绪、确定性红线、可选主动陪伴。
