...
### CASCADE 三级任务选择（避免空闲会话）

当 PRIMARY 队列空时，不立即进入规划模式，而是 CASCADE 到下一级：

| 级别 | 内容 | 示例 |
|------|------|------|
| **PRIMARY** | TODO.txt 待执行 + 用户请求 | 工具开发、冲浪任务、探测任务 |
| **SECONDARY** | 环境健康检查 | 定时任务运行状态、依赖完整性、磁盘/日志清理、索引漂移检测 |
| **TERTIARY** | 自改进/文档/代码质量 | SOP优化、代码注释补全、历史报告归档、memory准确性审计 |

**等待 vs 阻塞**：
- **waiting**（依赖未就绪但有其他可做）→ CASCADE 到下一优先级
- **blocked**（硬依赖，无替代路径）→ 才能结束本轮
- 禁止将"waiting"误判为"blocked"导致空闲
