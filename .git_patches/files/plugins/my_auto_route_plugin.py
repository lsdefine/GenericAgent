...
目标：把原有的 AutoRouting 逻辑集中到此插件，通过运行时 monkeypatch 注入到
`GeneraticAgent` 实例上，做到对项目源码 0 改动（仅在运行期修改行为）。

使用：在 `mykey.py` 中加入 `my_plugins = ['my_auto_route_plugin']`，
插件会在 `reload_mykeys()` 时被 import 并初始化。
"""
...
try:
    ...
    print(f"[MyAutoRoute] Init warning: missing dependencies: {e}")
    ModelRouter = None
    agentmain = None
...
def _ensure_attrs(agent):
    ...
    # router: 尝试从配置文件加载 auto_route_config.json
    router = None
    try:
        ...
        print(f"[MyAutoRoute] ModelRouter init failed: {e}")
        router = None
    ...
    # allowed free models: 来自 router 配置优先，否则默认集合
    try:
...
def _is_target_blocked(agent, target_name):
    ...
    if exp and time.time() > exp:
        try: del agent._myauto_blocked_targets[target_name]
        except: pass
        return False
    return True
