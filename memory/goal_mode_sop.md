...
用户给出开放目标 + 时间预算（如"最多花3小时持续优化X"、"在预算内尽量做好"），且不是一次性闭环任务。
...
- `budget_seconds`：预算上限，按用户要求调整；不要把预算当最低消费
...
- 产物达标时在回复最后一行单独输出 `[GOAL_READY_TO_WRAP]`，下一轮自动收口后停止
- 预算耗尽或超过 `max_turns` 时自动进入收口轮，然后停止
- 手动停：杀进程
