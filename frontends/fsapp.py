...
def _make_task_hook(card, task_id):
    """飞书任务 hook：每轮用 summary patch 卡片状态；不触发 task 完成。

    Finalization intentionally lives on the display-queue `done` item (and the
    timeout/stop/exception paths in ``run_agent``), not on a turn-level
    ``exit_reason`` from the agent. A turn-level exit_reason can fire on an
    intermediate turn and would otherwise finalize the queued task before the
    queue publishes its real ``done`` item, leaving the card marked complete
    with the wrong text. See #685.
    """
    def hook(ctx):
        try:
