...
- **`--task` 必须传 `temp/` 下的相对任务名**；传绝对路径时，源码会再拼 `temp/{args.task}`，Windows 下会变成 `temp/E:` 并报 `WinError 123`
...
- **复用旧 task 目录前先清空旧 `_stop`/`_intervene`/`output*.txt`/`reply*.txt`，或直接改用全新目录**；历史残留会污染轮询与行为判断
- **主agent空闲时应读output观察进度，必要时用干预文件纠偏，禁止无脑长时间sleep轮询**
- 若 `output.txt` 很快只出现`[ROUND END]`、但无 `result.md/reply.txt`，先核对相关文件 mtime，警惕旧目录残留或空转
- 若subagent持续读文件、无stderr、长时间不收敛，可向`_intervene`写入“停止继续读取，直接基于已读信息输出”的收敛指令
