#!/usr/bin/env python3
"""
Vision Pipeline 实战: 生成系统健康截图 → OCR 提取关键指标
"""
...
W, H = 640, len(lines) * 26 + 40
img = Image.new('RGB', (W, H), color=(20, 22, 36))
draw = ImageDraw.Draw(img)

try:
    font = ImageFont.truetype("/usr/share/fonts/dejavu/DejaVuSansMono-Bold.ttf", 18)
except:
    try:
...
y = 20
for line in lines:
    color = (0, 212, 170) if line.startswith('=') else (100, 200, 100) if '[UP]' in line else (255, 80, 80) if '[DOWN]' in line else (200, 200, 200)
    draw.text((20, y), line, fill=color, font=font)
    y += 24

screenshot_path = 'screenshots/system_health_ocr_test.png'
img.save(screenshot_path)
print(f"✅ Screenshot generated: {screenshot_path} ({W}x{H})")
print(f"   Lines in image: {len(lines)}")
...
for line in text.split('\n'):
    ...
    if 'Memory' in line:
        ...
        svc = line.split('[')[0].strip() if '[' in line else line
        status = 'UP' if 'UP' in line else 'DOWN'
        metrics[f'svc_{svc}'] = status

for k, v in metrics.items():
    print(f"  {k}: {v}")

print(f"\n✅ Total metrics extracted: {len(metrics)}")
print(f"✅ Pipeline verification: PASS")
