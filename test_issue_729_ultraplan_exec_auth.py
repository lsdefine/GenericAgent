...
# Resolve the repo root from this test file's location so the test works in
# any checkout (maintainer machine, CI, fresh clone) — not just the bot's.
import importlib.util, sys
_REPO_ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(_REPO_ROOT))
spec = importlib.util.spec_from_file_location("ga_ultraplan", _REPO_ROOT / "assets" / "ga_ultraplan.py")
...
# Cleanup: force the daemon's `while time() - _last < 3600` loop to exit by
# back-dating _last, then close the listening socket so the blocked
# handle_request() returns. (BaseServer.shutdown() only works against
# serve_forever() loops; against handle_request() it blocks forever.)
ga_ultraplan._last = 0.0
try: ga_ultraplan._srv.server_close()
