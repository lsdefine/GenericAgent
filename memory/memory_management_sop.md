...
L3: ../memory/ (记录库层 - 包含 .md/.py 等各类文件)
    └── tools/ (可 import 的通用工具模块 — weather_api/report_digest/chart_plotter/fzf_select/knowledge_lookup/notify)  
...
## 附录：L1 记忆整理与压缩（来自 memory_cleanup_sop，已合并）
...
### ROI 评估
L1两类内容统一按 ROI = (不放这几个词的犯错概率 × 代价) / 每轮词数成本 评估：
- **存在性指针**：指向L2/L3知识的最短触发词
- **行为规则**：不提醒就会犯的错（致命/高频均可，只要ROI过门槛）

### 压缩四原则
1. **命名自解释 > 加描述**：SOP名能说清的，L1不加注释
2. **存在性集合最小描述**：多个相近条目若可被同一上位场景覆盖，用集合名表达
3. **条目 = 场景↔方案存在性**：括号内只放反直觉触发词
4. **分层归位**：行为规则放上方场景行，纯存在性指针归L2/L3平铺列表

### 快速判断
- **该留**：反直觉触发词（没提示就想不到去查）
- **该删**：名字翻译、内容描述、直觉能力（不提醒也能想到）
